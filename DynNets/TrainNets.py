...
author, year, convs, kernels = sys.argv[1:]

file_name_args = [ author, year ]

if convs != '':
    for i in re.split(',', convs):
        file_name_args.append(i)
    convs = [ int(i) for i in re.split(',', convs) ]
else:
    convs = None

if kernels != '':
    for i in re.split(',', kernels):
        file_name_args.append(i)
    kernels = [ int(i) for i in re.split(',', kernels) ]
else:
    kernels = None

file_name = '_'.join(file_name_args)

set_writer(f'runs/train_{file_name}_{dt_string}')
writer = get_writer()
